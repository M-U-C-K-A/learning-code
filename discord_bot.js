            ...
            if (message.content.startsWith("LONG")) {
                ...
                (async function _iife() {
                            try {
                                ...
                                (async function() {
                                    try {
                                        // Tentative de sauvegarde de l'objet "Calls" en base de données
                                        await calls.save()
                                            .then(result => {
                                                // Si la sauvegarde réussit, affiche le résultat dans la console
                                                console.log(result);
                                            })
                                            .catch(err => {
                                                // Si la sauvegarde échoue, affiche l'erreur dans la console
                                                console.log(err);
                                            });
                                    } catch (err) {
                                        // Si une erreur se produit dans le bloc try/catch, affiche un message d'erreur dans la console
